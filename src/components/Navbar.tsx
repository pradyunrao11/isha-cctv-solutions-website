  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[#071A3D]/95 backdrop-blur-md shadow-lg shadow-blue-900/40 border-b border-blue-900'
          : 'bg-transparent'
      }`}
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-1">
          {navLinks.map((l) => (
            ...
            >
              {l.label}
              {active === l.href && (
                <span className="absolute inset-x-3 -bottom-0.5 h-0.5 bg-gradient-to-r from-[#12366A] to-yellow-400 rounded-full" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <button
            onClick={() => go('#contact')}
            className="px-5 py-2.5 rounded-lg bg-[#071A3D] text-white text-sm font-semibold shadow-lg shadow-blue-900/40 hover:shadow-blue-900/60 hover:-translate-y-0.5 transition-all"
          >
            Get Quote
          </button>
        </div>
        ...
        <button
          ...
          className="lg:hidden p-2 text-white rounded-lg hover:bg-[#0B2552] transition-colors"
      ...
      <div
        className={`lg:hidden overflow-hidden transition-all duration-300 bg-[#071A3D]/98 backdrop-blur-md border-t border-blue-900 ${
      ...
      >
        <div className="px-4 py-4 space-y-1">
          {navLinks.map((l) => (
            <button
              ...
              className={`block w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
                active === l.href ? 'bg-[#0B2552] text-yellow-300' : 'text-blue-100 hover:bg-[#0B2552]/60'
              }`}
            >
              {l.label}
            </button>
          ...
          <button
            onClick={() => go('#contact')}
            className="block w-full mt-2 px-4 py-3 rounded-lg bg-[#071A3D] text-white text-sm font-semibold text-center"
          >
            Get a Free Quote
          </button>
        </div>
